...
loss = 'output/loss_model_4.jpg'
acc = 'output/acc_model_4.jpg'
...
model.add(layers.Dense(300, input_dim=x.shape[1], activation="relu", name="layer1"))
model.add(layers.Dense(100, activation="relu", name="layer2"))
model.add(layers.Dense(y.shape[1], activation="softmax", name="layer3"))
...
pred_df.to_csv("output/" + dataset_number + "_" + dataset_chunk + "model_4_predictions.csv")
test_labels = pd.DataFrame(y_t)
test_labels.to_csv("output/" + dataset_number + "_" + dataset_chunk + "model_4_predictions_labels.csv")
